...
public final class LoggerFactory {
    ...
    /**
     * Initialisation de la factory de logger.
     * 
     * @param cl
     *            ClassLoader
     * @return Vrai si initialisation OK
     */
    private static boolean initialize(final ClassLoader cl) {
        ...
        if (configFileResource != null) {
            ...
            String property = getBundleString(configFileResource, Log4jProperties.LOG_DIR);
            if (property != null && property.length() > 0) {
                defaultLogDir = new File(property.trim());
            }

            // Look for the property ROLLING_MAX_BACKUP_INDEX
            property = getBundleString(configFileResource, Log4jProperties.ROLLING_MAX_BACKUP_INDEX);
            if (property != null && property.length() > 0) {
                try {
            ...
            property = getBundleString(configFileResource, Log4jProperties.ROLLING_MAX_FILE_SIZE);
            if (property != null && property.length() > 0) {
                defaultMaxFileSize = property.trim();
            }

            // Look for the property LOG_ENCODING
            property = getBundleString(configFileResource, Log4jProperties.LOG_ENCODING);
            if (property != null && property.length() > 0) {
                defaultEncoding = property.trim();
            }

            // Look for the property LOG_PATTERN
            property = getBundleString(configFileResource, Log4jProperties.LOG_PATTERN);
            if (property != null && property.length() > 0) {
        ...
        if (defaultLogDir != null && !defaultLogDir.exists()) {
     ...
     *            resource bundle
     * @param property
     *            proprité à récupérer
     * @return value valeur de la propriété
     */
    private static String getBundleString(final ResourceBundle bundle, final String property) {
     ...
     */
    private static RollingFileAppender getRollingAppender(final String filename) throws IOException {
     ...
     */
    public static ILogger createLogger(final String componentName, final String loggerName) {
        ...
        try {
            ...
            MDC.put(Log4jProperties.MDC_KEY_HOSTNAME, addr.getHostName());
            MDC.put(Log4jProperties.MDC_KEY_HOSTADDRESS, addr.getHostAddress());
        } catch (UnknownHostException e) {
            MDC.put(Log4jProperties.MDC_KEY_HOSTNAME, "localhost");
            MDC.put(Log4jProperties.MDC_KEY_HOSTADDRESS, "127.0.0.1");
        }
        ...
        String filename =
                ...
                defaultLogDir.getAbsolutePath() + File.separator
                        + MDC.get(Log4jProperties.MDC_KEY_HOSTNAME).toString().toUpperCase() + '_'
                        + componentName.toUpperCase() + ".LOG";
     ...
     * @return A brand new logger able to log messages on the local machine.
     */
    public static ILogger createLogger(final String componentName, final String loggerName, final File logDirectory,
            final String outputFileName) {
        String myOutputFileName = outputFileName;
        if (!isInitialized) {
            isInitialized = initialize(ClassLoader.getSystemClassLoader());
        }
        if (!isInitialized) {
        ...
        // destination des logs. Donc on remet 'myOutputFileName' en relatif.
        if (myOutputFileName.contains("/")) {
            myOutputFileName = myOutputFileName.substring(myOutputFileName.lastIndexOf('/') + 1);
        }
        if (myOutputFileName.contains("\\")) {
            myOutputFileName = myOutputFileName.substring(myOutputFileName.lastIndexOf('\\') + 1);
        }
        myOutputFileName = logDirectory.getAbsolutePath() + File.separator + MDC.get(Log4jProperties.MDC_KEY_HOSTNAME)
                + '_' + myOutputFileName;
        ...
        if (existingLogger.getAppender(ROLLING_FILE_APPENDER) != null) {
            ...
            if (appender instanceof RollingFileAppender) {
                ...
                rollingAppender.setFile(myOutputFileName);
        ...
        if (rootAppender != null && rootAppender instanceof FileAppender) {
            ...
            ((FileAppender) rootAppender).setFile(myOutputFileName);
            ((FileAppender) rootAppender).activateOptions();
        } else {
            // On ajoute un nouveau FileAppender au logger de l'utilisateur
            try {
                existingLogger.addAppender(getRollingAppender(myOutputFileName));
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
     ...
     * @return A brand new logger able to log messages on the local machine.
     */
    public static ILogger createLogger(final ClassLoader cl, final String componentName, final String loggerName) {
        // A partir du rootLogger, on recupere le repository afin de savoir
        // si l'utilisateur a fourni un fichier de conf log4j avec un logger
        // nomme '<loggerName>'. Si oui, alors on n'influe pas du tout sur son
        // objet. Par contre si aucun logger de ce nom n'existe dans le fichier
     ...
     */
    public static ILogger createLogger(final ClassLoader cl, final String componentName, final String loggerName,
            final File logDirectory, final String outputFileName) {
        String myOutputFileName = outputFileName;
        if (!isInitialized) {
            isInitialized = initialize(cl);
        }
        ...
        if (existingLogger == null) {
            // Il n'existe pas donc on en crï¿½e un nouveau
            existingLogger = Logger.getLogger(loggerName);
            existingLogger.setAdditivity(false);
        }

        // Si le logger a dï¿½jï¿½ un appender "RollingFileAppender", pas la
        // peine
        // d'ajouter le notre encore une fois. Par contre il faut modifier le
        // fichier de sortie pour que les traces aillent dans le fichier
        // demandï¿½.
        ...
        if (rootAppender != null && rootAppender instanceof FileAppender) {
            // l'utilisateur a deja un RollingFileAppender associe au
            // rootLogger.
            // On prend celui lï¿½ et on lui change juste le nom de fichier
            ((FileAppender) rootAppender).setFile(myOutputFileName);
            ((FileAppender) rootAppender).activateOptions();
        } else {
            // On ajoute un nouveau FileAppender au logger de l'utilisateur
